...
export async function createTeam(param: {
  userId: string;
  name: string;
  slug: string;
}) {
...
export async function getTeam(key: { id: string } | { slug: string }) {
  return await prisma.team.findUniqueOrThrow({
    where: key,
  });
};

export async function deleteTeam(key: { id: string } | { slug: string }) {
...
export async function updateTeam(slug: string, data: Partial<Team>) {
...
*/
export async function isTeamExists(slug: string){
...
export async function getTeamMember(userId: string, slug: string) {
...
export async function getUniqueTeamContact(id: string, teamId: string, withLabel: boolean) {
    return await prisma.teamContact.findUnique({
        where: {id: id, teamId: teamId},
        include: {ContactLabel: withLabel}
    });
}

export async function getAllTeamContacts(teamId: string, withLabel: boolean, where: any) {
    // add queries
    // - by label
    // - by DDD (?)
    ...
    if (!withLabel) return await prisma.teamContact.findMany({
        where: where 
    })

    return await prisma.teamContact.findMany({
        where: where,
        include: {
            ContactLabel: {
                select: {
...
export async function createTeamContact(data: any, teamId: string) {
    return await prisma.teamContact.create({
        data:{
            ...data,
            teamId: teamId
        },
...
export async function updateTeamContact(id: string, teamId: string, data: any) {
    return await prisma.teamContact.update({
       where: {id: id, teamId: teamId}, 
       data: data
    });
}

export async function deleteUniqueTeamContact(id: string, teamId: string) {
    return await prisma.teamContact.delete({
        where: {id: id, teamId: teamId}
    });
}
